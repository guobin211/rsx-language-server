...
describe('LSP Server Initialization', () => {
    ...
    test('should support completion', async () => {
        ...
        assert.ok(Array.isArray(result.capabilities.completionProvider.triggerCharacters), 
            'Trigger characters should be an array');
        assert.ok(result.capabilities.completionProvider.triggerCharacters.length > 0,
    ...
    test('should support semantic tokens', async () => {
        ...
        assert.ok(Array.isArray(result.capabilities.semanticTokensProvider.legend.tokenTypes),
            'Token types should be an array');
        assert.ok(Array.isArray(result.capabilities.semanticTokensProvider.legend.tokenModifiers),
